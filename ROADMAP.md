...
8. Multi-query `find` and multi-range `read_range` (§2)
...
- [ ] **Name the checker in every edit response** — `checked: gopls`,
...
- [ ] **Language-server diagnostics after edits for every configured
...
- [ ] **Stop reporting transient errors from half-done multi-step edits.**
  ...
  same edit just introduced or removed.
