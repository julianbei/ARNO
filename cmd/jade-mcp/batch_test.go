...
func TestFindAnswersSeveralNamesInOneCall(t *testing.T) {
	...
	out, err := callText(t, server, "jade.find", map[string]interface{}{
...
func TestFindWithNeitherQueryNorQueriesSaysWhatItNeeds(t *testing.T) {
	...
	_, err := server.handleToolCall(toolCall(t, "jade.find", map[string]interface{}{}))
...
func TestReadRangeStartPastTheFileIsStillAnError(t *testing.T) {
	...
	if _, err := callText(t, server, "jade.read_range", map[string]interface{}{"path": "short.txt", "startLine": 5}); err == nil {
