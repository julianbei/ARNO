...
func TestTwoSessionsOnOneWorkspaceDoNotLoseEachOthersWork(t *testing.T) {
	...
	read, err := callText(t, first, "jade.read_range", map[string]interface{}{"path": "shared.txt"})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := callText(t, second, "jade.replace_text", map[string]interface{}{"path": "shared.txt", "oldText": "beta", "newText": "BETA"}); err != nil {
		t.Fatal(err)
	}

	_, err = callText(t, first, "jade.replace_text", map[string]interface{}{
	...
	fresh, err := callText(t, first, "jade.read_range", map[string]interface{}{"path": "shared.txt"})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := callText(t, first, "jade.replace_text", map[string]interface{}{
	...
	for i, job := range []struct {
		...
		go func(i int, server *mcpServer, path string, old string) {
			defer wg.Done()
			_, errs[i] = callText(t, server, "jade.apply", map[string]interface{}{
				"edits": []interface{}{
