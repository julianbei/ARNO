...
func TestGrepBudgetPagesWithContinue(t *testing.T) {
	...
	first, err := callText(t, server, "arno.grep", map[string]interface{}{"query": "needle", "budget": 40})
	if err != nil {
		t.Fatal(err)
	}
	...
	next := handle[1]
	for i := 0; i < 30 && next != ""; i++ {
		page, err := callText(t, server, "arno.grep", map[string]interface{}{"continue": next})
		if err != nil {
...
func TestGrepContinueRefusedAfterAnEdit(t *testing.T) {
	...
	if _, err := callText(t, server, "arno.replace_text", map[string]interface{}{"path": "a.txt", "oldText": "needle 01", "newText": "needle 00"}); err != nil {
		t.Fatal(err)
	}
	if _, err := callText(t, server, "arno.grep", map[string]interface{}{"continue": handle[1]}); err == nil || !strings.Contains(err.Error(), "repeat the call") {
...
func TestFindBudgetPagesWithContinue(t *testing.T) {
	...
	page, err := callText(t, server, "arno.find", map[string]interface{}{"query": "Needle", "budget": 60})
	if err != nil {
		t.Fatal(err)
	}
	all := page
	for i := 0; i < 40; i++ {
		...
		if page, err = callText(t, server, "arno.find", map[string]interface{}{"continue": m[1]}); err != nil {
			t.Fatal(err)
		}
		all += "\n" + page
	}
...
func TestReferencesBudgetPagesWithContinue(t *testing.T) {
	...
	page, err := callText(t, server, "arno.references", map[string]interface{}{"path": "a.go", "symbolName": "Target", "budget": 20})
	if err != nil {
		t.Fatal(err)
	}
	all := page
	for i := 0; i < 40; i++ {
		...
		if page, err = callText(t, server, "arno.references", map[string]interface{}{"continue": m[1]}); err != nil {
			t.Fatal(err)
		}
		all += "\n" + page
	}
...
func TestReadRangePagesALargeFileInsteadOfCuttingItsMiddle(t *testing.T) {
	...
	page, err := callText(t, server, "arno.read_range", map[string]interface{}{"path": "big.txt"})
	if err != nil {
		t.Fatal(err)
	}
	...
	for i := 0; i < 20; i++ {
		...
		if page, err = callText(t, server, "arno.read_range", map[string]interface{}{"continue": m[1]}); err != nil {
			t.Fatal(err)
		}
		all += "\n" + page
	}
...
func TestWorkspaceTreeBudgetPagesWithContinue(t *testing.T) {
	...
	page, err := callText(t, server, "arno.workspace_tree", map[string]interface{}{"budget": 20})
	if err != nil {
		t.Fatal(err)
	}
	all := page
	for i := 0; i < 60; i++ {
		...
		if page, err = callText(t, server, "arno.workspace_tree", map[string]interface{}{"continue": m[1]}); err != nil {
			t.Fatal(err)
		}
		all += "\n" + page
	}
...
func TestReadRangesPageALargeRangeWithAReadRangeHandle(t *testing.T) {
	...
	rest, err := callText(t, server, "arno.read_range", map[string]interface{}{"continue": m[1]})
	if err != nil {
		t.Fatal(err)
	}
