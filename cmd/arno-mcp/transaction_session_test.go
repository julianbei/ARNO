...
func TestTheChangeTransactionHoldsAcrossAScriptedSession(t *testing.T) {
	...
	first := checkpointID.FindString(must("arno.checkpoint", map[string]interface{}{"note": "before the session"}))
	if first == "" {
		t.Fatal("checkpoint returned no id")
	}
	must("arno.create_file", map[string]interface{}{"path": "new.txt", "content": "created\n"})
	must("arno.delete_file", map[string]interface{}{"path": "old.txt"})

	// A multi-file apply lands in both files.
	must("arno.apply", map[string]interface{}{"edits": []interface{}{
	...
	if _, err := call("arno.replace_text", map[string]interface{}{"path": "a.txt", "oldText": "ALPHA", "newText": "Alpha", "expectedDigest": digest[1]}); err == nil || !strings.Contains(err.Error(), "changed since the read") {
	...
	must("arno.declare_command", map[string]interface{}{"name": "verify", "run": "grep -q fixed status.txt", "kind": "lint"})
	if out := must("arno.check", map[string]interface{}{"kind": "lint"}); !strings.HasPrefix(firstLine(out), "FAIL") {
		t.Fatalf("the check should fail before the fix:\n%s", out)
	}
	must("arno.replace_text", map[string]interface{}{"path": "status.txt", "oldText": "broken", "newText": "fixed"})
	if out := must("arno.check", map[string]interface{}{"kind": "lint"}); !strings.HasPrefix(firstLine(out), "pass") {
	...
	if _, err := call("arno.revert", map[string]interface{}{"checkpointId": first}); err == nil || !strings.Contains(err.Error(), "commit") {
	...
	second := checkpointID.FindString(must("arno.checkpoint", map[string]interface{}{"note": "after the commit"}))
	must("arno.replace_text", map[string]interface{}{"path": "status.txt", "oldText": "fixed", "newText": "broken again"})
	must("arno.create_file", map[string]interface{}{"path": "extra.txt", "content": "extra\n"})
	must("arno.delete_file", map[string]interface{}{"path": "b.txt"})
	must("arno.revert", map[string]interface{}{"checkpointId": second})
	...
	if changes := must("arno.changes", map[string]interface{}{}); !strings.Contains(changes, "ran: ") || !strings.Contains(changes, "check lint") {
