...
var toolAnnotationsByName = map[string]toolAnnotations{
	...
	"arno.outline":         readOnlyTool,
	"arno.read_range":      readOnlyTool,
	"arno.history":         readOnlyTool,
	"arno.context":         readOnlyTool,
	"arno.references":      readOnlyTool,
	"arno.find":            readOnlyTool,
	"arno.retrieve":        readOnlyTool,
	"arno.grep":            readOnlyTool,
	"arno.telemetry":       readOnlyTool,
	"arno.changes":         readOnlyTool,
	"arno.diff":            readOnlyTool,
	...
	"arno.check":           additiveTool,
	"arno.run_tests":       additiveTool,
	"arno.replace_symbol":  destructiveTool,
	"arno.replace_text":    destructiveTool,
	"arno.replace_file":    destructiveTool,
	"arno.apply":           destructiveTool,
	"arno.delete_symbol":   destructiveTool,
	"arno.delete_file":     destructiveTool,
	"arno.rename":          destructiveTool,
	"arno.revert":          destructiveTool,
	"arno.run_command":     destructiveTool,
	"arno.declare_command": destructiveTool,
}
...
type mcpServer struct {
	...
	// repository's .arno/project.json declares. Empty sends the server's own.
	instructions string
...
func main() {
	...
	for _, arg := range os.Args[1:] {
		if arg == "--version" || arg == "-version" || arg == "version" {
			fmt.Fprintf(os.Stdout, "arno-mcp %s\n", resolveVersion())
	...
	fmt.Fprintf(os.Stderr, "arno-mcp %s · workspace %s (from %s)\n", resolveVersion(), resolved.Path, resolved.Source)
	if resolved.Warning != "" {
		fmt.Fprintf(os.Stderr, "arno-mcp warning: %s\n", resolved.Warning)
	}
...
func (s *mcpServer) dispatchToolCall(name string, args map[string]interface{}) (mcpToolResult, error) {
	...
	case "arno.capabilities":
		...
		if err == nil && len(result.Content) > 0 {
			...
			header := "arno-mcp " + resolveVersion()
			if notice := update.New(resolveVersion()).Notice(); notice != "" {
				header += "\n" + notice
			}
...
func catalogTools() []mcpTool {
	return []mcpTool{
		...
		},
		{
			...
			InputSchema: map[string]interface{}{
				...
				"properties": map[string]interface{}{
					...
					"check":            map[string]interface{}{"type": "string", "description": "Run one validation after all edits: build, typecheck, tests (the edited files' tests), or impact (those plus tests of callers of touched declarations)."},
				},
				"required": []string{"edits"},
			},
		},
		{
			Name:        "arno.delete_symbol",
			Description: "Delete one declaration entirely. The range comes from arno's parse, so the caller never has to find the closing brace; the blank line the declaration left behind is removed too.",
			InputSchema: map[string]interface{}{
				"type": "object",
				"properties": map[string]interface{}{
		...
		{
			Name:        "arno.telemetry",
			Description: "Report how arno's own tools have been used in this workspace: calls, response bytes and timing per tool, plus the failure classes that most often end with a caller falling back to the shell. Records no arguments, no response bodies and no error text.",
			InputSchema: map[string]interface{}{
				"type": "object",
				"properties": map[string]interface{}{
		...
		{
			Name:        "arno.declare_command",
			Description: "Declare a named command in .arno/commands.json — a reproduction, a benchmark — to run with run_command in this and later sessions; the file is reviewed like any change. Declaring an existing name replaces it; do not redeclare one just to run it.",
			InputSchema: map[string]interface{}{
				"type": "object",
				"properties": map[string]interface{}{
...
// editOpsArg decodes the edits array for arno.apply. It round-trips through
...
// arnoJSONOutput forces the legacy JSON wire format. Plain text is the
