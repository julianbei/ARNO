...
func TestToolCallsAreRecordedThroughTheRealDispatchPath(t *testing.T) {
	...
	if _, err := server.handleToolCall(toolCall(t, "arno.changes", nil)); err != nil {
	...
	if summary.Tools[0].Tool != "arno.changes" {
...
func TestFailedToolCallsAreRecordedWithTheirClass(t *testing.T) {
	...
	if _, err := server.handleToolCall(toolCall(t, "arno.job_status", map[string]interface{}{
...
func TestUnknownToolIsRecordedToo(t *testing.T) {
	...
	server, root := newTestMCPServer(t)
	if err := os.MkdirAll(filepath.Join(root, ".arno"), 0o755); err != nil {
		t.Fatal(err)
	}
	...
	if summary.TotalCalls != 1 || summary.Tools[0].Tool != "arno.does_not_exist" {
...
func TestTelemetryToolReportsWhatWasRecorded(t *testing.T) {
	...
	result, err := server.handleToolCall(toolCall(t, "arno.telemetry", nil))
	if err != nil {
	...
	if !strings.Contains(text, "arno.changes") {
...
func TestTelemetryResetClearsTheLogThroughTheTool(t *testing.T) {
	...
	if _, err := server.handleToolCall(toolCall(t, "arno.telemetry", map[string]interface{}{
		"reset": true,
	})); err != nil {
...
func TestTelemetryWritesOnlyUnderDotArno(t *testing.T) {
...
func TestInBandFailureIsRecordedAsAFallback(t *testing.T) {
	...
	// was removed.
	if _, err := server.handleToolCall(toolCall(t, "arno.check", map[string]interface{}{
		"kind": "build",
	})); err != nil {
