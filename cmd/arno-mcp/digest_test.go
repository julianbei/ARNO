...
func TestAnEditIsRefusedWhenTheFileChangedSinceItsRead(t *testing.T) {
	...
	read, err := callText(t, server, "arno.read_range", map[string]interface{}{"path": "a.txt"})
	if err != nil {
		t.Fatal(err)
	}
	...
	_, err = callText(t, server, "arno.replace_text", map[string]interface{}{
	...
	fresh, err := callText(t, server, "arno.read_range", map[string]interface{}{"path": "a.txt"})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := callText(t, server, "arno.replace_text", map[string]interface{}{
