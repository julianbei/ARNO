...
func TestDependencySourceIsReadableNotWritable(t *testing.T) {
	...
	found, err := callText(t, server, "arno.grep", map[string]interface{}{"query": "leftPad", "dependency": "left-pad"})
	if err != nil {
		t.Fatalf("grep: %v", err)
	}
	...
	read, err := callText(t, server, "arno.read_range", map[string]interface{}{"path": "dep:left-pad/index.js", "lines": "1-2"})
	if err != nil || !strings.Contains(read, "function leftPad") {
		t.Errorf("expected the dependency file, got %v:\n%s", err, read)
	}

	if _, err := callText(t, server, "arno.replace_text", map[string]interface{}{"path": "dep:left-pad/index.js", "oldText": "return str;", "newText": "return ch;"}); err == nil {
		t.Error("a dependency path must not be editable")
	}

	if _, err := callText(t, server, "arno.grep", map[string]interface{}{"query": "x", "dependency": "../outside"}); err == nil {
