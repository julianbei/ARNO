...
func TestCheckResultNamesTheCommandThatRan(t *testing.T) {
	...
	out, err := callText(t, server, "arno.check", map[string]interface{}{"kind": "build", "timeoutSeconds": 120})
	if err != nil {
...
func TestCheckWithNoRecognisableProjectSaysSoUpFront(t *testing.T) {
	...
	out, err := callText(t, server, "arno.check", map[string]interface{}{"kind": "tests"})
	if err != nil {
		t.Fatalf("check: %v", err)
	}
