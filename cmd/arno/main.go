...
import (
	...
	"github.com/julianbei/arno/internal/transport/mcp"
...
func main() {
	...
	if len(os.Args) == 3 && os.Args[1] == "api-go-parser-compare" {
		_ = os.Setenv("ARNO_GO_SYMBOL_PARSER", "regex")
		regexSymbols, regexErr := ci.Outline(os.Args[2])
		if regexErr != nil {
			log.Fatalf("regex outline failed: %v", regexErr)
		}

		_ = os.Setenv("ARNO_GO_SYMBOL_PARSER", "tree-sitter")
	...
	log.Println("arno runtime initialized")
}
