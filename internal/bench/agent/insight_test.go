...
// one-call shell run and a one-call Jade run, with local paths and settings
// removed.
...
func TestAnalyzeReadsARealShellTranscript(t *testing.T) {
	...
	if !in.Valid() || in.JadeStatus != "" {
		t.Errorf("a shell run is valid without Jade: %+v", in)
	}
}

func TestAnalyzeReadsARealJadeTranscript(t *testing.T) {
	in, err := Analyze(RunResult{Arm: ArmJade, Transcript: "testdata/stream-jade.jsonl"})
	if err != nil {
		t.Fatal(err)
	}
	if in.JadeStatus != "connected" || !in.Valid() {
		t.Errorf("expected Jade connected, got %q", in.JadeStatus)
	}
	if len(in.Calls) != 1 || in.Calls[0].Tool != "jade.outline" || in.Calls[0].Category != CategoryRead || in.Calls[0].Target != "main.go" {
		t.Fatalf("expected one jade.outline read of main.go, got %+v", in.Calls)
	}
	if in.Calls[0].ResultBytes == 0 {
		t.Error("the Jade result's size was not joined to its call")
	}
}

func TestAJadeArmWithoutJadeIsInvalid(t *testing.T) {
	path := filepath.Join(t.TempDir(), "t.jsonl")
	if err := os.WriteFile(path, []byte(`{"type":"system","subtype":"init","mcp_servers":[{"name":"jade","status":"failed"}]}`+"\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	in, err := Analyze(RunResult{Arm: ArmJade, Transcript: path})
	if err != nil {
		t.Fatal(err)
	}
	if in.Valid() {
		t.Fatal("a Jade arm whose server failed must not count as a Jade result")
	}
}
...
func TestAnalyzeDerivesTheCallSequenceMetrics(t *testing.T) {
	lines := []string{
		...
		`{"type":"assistant","request_id":"r1","message":{"content":[{"type":"tool_use","id":"b","name":"mcp__jade__jade_find","input":{"query":"Run"}}],"usage":{"input_tokens":10,"cache_read_input_tokens":1000,"output_tokens":9}}}`,
		...
		`{"type":"assistant","request_id":"r3","message":{"content":[{"type":"tool_use","id":"d","name":"mcp__jade__jade_replace_text","input":{"path":"main.go","oldText":"a","newText":"b"}}]}}`,
	...
	in, err := Analyze(RunResult{Arm: ArmJadeShell, Transcript: path})
	if err != nil {
		t.Fatal(err)
	}
	...
	if in.Calls[1].Tool != "jade.find" || in.Calls[1].Category != CategorySearch {
		t.Errorf("jade tool names should normalise: %+v", in.Calls[1])
	}
}
...
func TestInsightReportComparesArmsPerTask(t *testing.T) {
	results := []RunResult{
		...
		{Repository: "demo", Task: "t1", Arm: ArmJade, Repeat: 1, Success: true, Turns: 20, Transcript: "testdata/stream-jade.jsonl"},
		{Repository: "demo", Task: "t1", Arm: ArmJade, Repeat: 2, Success: false, Turns: 30, Transcript: "testdata/stream-jade.jsonl"},
