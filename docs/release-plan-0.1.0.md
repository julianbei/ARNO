...
`.arno/commands.json` as a repository vocabulary — and three cautions:
...
- [x] **External benchmark with bash as the baseline** (ROADMAP § Evidence).
  ...
  the task did not need. *Done when* `arno-bench` can run
...
- [ ] **Host integrations, in order:** OpenCode, goose, Cline, Codex CLI,
  ...
  *Progress 2026-09-14:* **Codex CLI 0.154 verified** (find, insert, check
  ...
  named `arno_arno_*`); README "Other hosts" has all three. Cline and Gemini
  CLI not installed.
 ...
 *Progress 2026-09-14:* `TestDegraded`
  ...
  languages without the container, and passes in it; `TestSemantics` fails
  over a per-language first-answer budget set from the container baseline.
...
- [x] **Strengthen declared commands.** They are how validation stays inside
  ...
  one record.
  *Progress 2026-09-14:* commands carry a `kind`; `check lint|codegen` runs
...
- [x] **Validation chain through declared commands.** *Done 2026-09-14:*
  ...
  `.arno/commands.json` that runs Semgrep (or any repository rule tool) after
...
counted on [issue #2](https://github.com/julianbei/arno/issues/2). A port would
...
- **Out-of-process plugins**, built on Phase 4's registry and Phase 5's single
  ...
  - Subprocesses over stdio JSON-RPC, a protocol far smaller than MCP:
    ...
    capability (`arno.capability.references/v1`…). A plugin may provide any
    subset.
  ...
  - Package split only when there is a reason. Language support and its
    ecosystem ship together (`arno-lang-rust` includes Cargo discovery).
