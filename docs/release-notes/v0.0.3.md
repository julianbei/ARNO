...
- **Paths outside the workspace are refused.** An absolute path elsewhere, a
  ...
  they still work.
- **Validation results lead with one of five words:** `pass`, `FAIL`,
  `unavailable`, `running` or `timed out`. A check that outlives its wait
  says `timed out` instead of `running`; a missing tool says `unavailable`
  instead of `FAIL`. JSON output gains an `Outcome` field.
...
- [x] Item 12: typed validation outcomes
